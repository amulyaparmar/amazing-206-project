...
cur.execute("SELECT * FROM WebsiteData WHERE category='Daily Visitors'")
df = pd.DataFrame(cur.fetchall())

df = df.rename(columns={1: "Candidate", 2: 'Website Statistic', 3: "Daily Visitors (per 1k)"}, errors="raise")


df = df.set_index("Candidate")

df["Daily Visitors (per 1k)"] = df["Daily Visitors (per 1k)"]  / 1000
...
Gtrend_DELTA as t1
INNER JOIN
(SELECT * FROM WebsiteData WHERE category='Daily Visitors') as t2
ON t1.name = t2.candidate

""")

pd.DataFrame(cur.fetchall()).set_index(1).iloc[:,1:].loc[]
